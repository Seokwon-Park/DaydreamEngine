...
namespace Daydream
{
	VulkanRenderPass::VulkanRenderPass(VulkanRenderDevice* _device, const RenderPassDesc& _desc)
	{
		...
		for (size_t i = 0; i < _desc.colorAttachments.size(); ++i)
		{
			...
			colorAttachment.format = GraphicsUtility::Vulkan::ConvertToVkFormat(_desc.colorAttachments[i].format);
		...
		if (hasDepth)
		{
			...
			depthAttachment.format = GraphicsUtility::Vulkan::ConvertToVkFormat(_desc.depthAttachment.format);
